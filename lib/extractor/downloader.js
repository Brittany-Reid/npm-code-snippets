...
class Downloader {
	 ...
	 */
	static async download(url, token) {
		...
		return new Promise(function (resolve, reject) {
			var request =  protocol.get(url, options, function (response) {
				if(response.statusCode === 403){
					if(response.headers["x-ratelimit-reset"] && response.headers["x-ratelimit-remaining"] === '0'){
						this.destroy(); //KILL IT WITH FIRE
						reject(new HTTPStatusError("code " + response.statusCode + " for url " + url, url, response.statusCode, response.headers["x-ratelimit-reset"]));
					}
				}
	...
	async getReadmeFromRepo(registryEntry){
		...
		if(url.includes("github.com/")){
			raw = Downloader.processGithubURL(url);
		}
		else{
...
class HTTPStatusError extends Error{
	 ...
	 */
	constructor(message, url, statusCode, wait){
		super(message);
